...
sub _build__gmtime_attrs {
    ...
    @params{
        qw(second minute hour day_of_month month year day_of_week day_of_year)}
      = gmtime( $self->{epoch} );
...
sub _build_date_ddmmmyyyy {
    ...
    return
      join( '-', ( $self->day_of_month, $self->month_as_string, $self->year ) );
}
...
sub _build_date_ddmmmyy {
    ...
    return join(
        '-',
        (
            $self->day_of_month, $self->month_as_string,
            $self->year_in_two_digit
        )
    );
}
...
sub new {
    ...
    if ( not defined $params_ref ) {
        ...
        if ( not( $params_ref->{'datetime'} or $params_ref->{epoch} ) ) {
            confess
'Must pass either datetime or epoch to the Date object constructor';
        }
        elsif ( $params_ref->{'datetime'} and $params_ref->{epoch} ) {
            confess
'Must pass only one of datetime or epoch to the Date object constructor';
        }
...
my $fully_specced =
qr/^([12][0-9]{3})-?([01]?[0-9])-?([0-3]?[0-9])(?:T|\s)?([0-2]?[0-9]):?([0-5]?[0-9]):?([0-5]?[0-9])(\.[0-9]+)?(?:Z)?$/;
my $numeric_date_only_dd_mm_yyyy =
  qr/^([0-3]?[0-9])-([01]?[0-9])-([12][0-9]{3})$/;
...
sub _parse_datetime_param {
    ...
    if ( length $year == 2 ) {
        if ( $year > 30 and $year < 70 ) {
            croak
'Date::Utility only supports two-digit years from 1970-2030. We got ['
              . $year . ']';
        }
...
sub _build_date_ddmmyy {
    ...
    return join(
        ...
        (
            sprintf( '%02d', $self->day_of_month ),
            sprintf( '%02d', $self->month ),
            sprintf( '%02d', $self->year_in_two_digit )
        )
    );
}
...
sub _build_days_in_month {
    ...
    if ( $month == 2 ) {
        my $year = $self->year;
        $last_day =
          ( ( $year % 4 or not $year % 100 ) and ( $year % 400 ) ) ? 28 : 29;
    }
...
sub _move_time_interval {
    ...
    return ( $sec == 0 )
      ? $self
      : Date::Utility->new( $self->{epoch} + $dir * $sec );
}
...
sub move_to_nth_dow {
    ...
    my $dow = $days_to_num{ lc $dow_abb }
      // croak 'Invalid day of week. We got [' . $dow_abb . ']';

    my $dow_first =
      ( 7 - ( $self->day_of_month - 1 - $self->day_of_week ) ) % 7;
    my $dom = ( $dow + 7 - $dow_first ) % 7 + ( $nth - 1 ) * 7 + 1;

    return
      try { Date::Utility->new( join '-', $self->year, $self->month, $dom ) };
}
