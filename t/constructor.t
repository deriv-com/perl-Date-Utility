...
throws_ok { Date::Utility->new( {} ) } qr/Must pass either datetime or epoch/,
  'empty parameters, no object';

# Faily stuff
throws_ok { Date::Utility->new( { datetime => 'fake', epoch => 1 } ) }
qr/Must pass only one of datetime or epoch/, 'both epoch and datetime';
throws_ok { Date::Utility->new( datetime => 'fake', epoch => 1 ); }
qr/Invalid datetime format/, 'params not in a hash ref';
throws_ok { Date::Utility->new( { datetime => '991111' } ); }
qr/Invalid datetime format/,
  'numeric string as supplied date time is neither 8 nor 14 chars long';

# New style faily stuff
throws_ok { Date::Utility->new('4-Jun-20001') } qr/Invalid datetime format/,
  'invalid year part';
throws_ok { Date::Utility->new("\N{THAI DIGIT FOUR}-Jun-2001") }
qr/Invalid datetime format/, 'unicode digits are not accepted';
throws_ok { Date::Utility->new("4-Juu-2001") } qr/Invalid datetime format/,
  'failed on invalid month';
throws_ok { Date::Utility->new("2001-12-44") } qr/Invalid datetime format/,
  'failed on 44th dec';
throws_ok { Date::Utility->new("2001-12-31 23:59:61") }
qr/Invalid datetime format/, 'not accepting 61 as a value for seconds';
throws_ok { Date::Utility->new('4-Jun-54') }
qr/only supports two-digit years from 1970-2030/,
  'No two digit years between 30 and 70';
throws_ok { Date::Utility->new('fake') } qr/Invalid datetime format/,
  'random string is not a date string';
...
subtest 'leap years' => sub {
    throws_ok { Date::Utility->new('2001-02-29') } qr/Day '29' out of range/,
      'No leap day in 2001';
    throws_ok { Date::Utility->new('1900-02-29') } qr/Day '29' out of range/,
      '... nor in 1900 by the 100 rule';
    new_ok(
